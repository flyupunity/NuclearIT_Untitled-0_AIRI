...
Модель обучена на `N=256` уникальных погодных состояниях и проверена на
16 тестовых кадрах 2021 года.
...
| Natural-variability criteria | **8/8, PASS** |

Калибровка выбрала `gain=170.8232`. Требование `CR ≥ 32×` выполнено на каждом
тестовом кадре, а не только в среднем.
...
В опубликованном execution был загружен совместимый checkpoint предыдущего
запуска этого же решения. В checkpoint сохранены:

- 12 000 optimizer steps;
- 24 000 обработанных обучающих примеров;
- `N=256` уникальных train-кадров.

Это повторное использование собственного checkpoint, а не внешнее
предобучение на погодных данных.
...
В опубликованном запуске probe был полностью вычислен:

| Параметр probe | Значение |
|---|---:|
| Train pairs | 1024 уникальные пары |
| Validation pairs | 32 |
| Test pairs | 32 |
| Обучаемые параметры | 194 496 |
| Optimizer steps | 5000 |
| Best validation latent MSE | 0.000708 |
| Улучшение против persistence на validation | 20.019% |
| Улучшение против persistence на test | 19.029% |
| Статус | `evaluated_on_test_2021` |

Codec во время обучения probe заморожен. Поэтому результат показывает, что
сжатый латент сохраняет полезную динамическую информацию, а не только позволяет
...
Опубликованная версия содержит 91 выходной файл. Главный submission archive
имеет размер около `39.44 MiB` и включает checkpoint, runtime-код, конфигурацию,
статистики, bitstream-примеры, метрики и probe-артефакты.

## Фактические ресурсы опубликованного запуска

| Ресурс | Значение |
|---|---:|
| GPU | Tesla P100-PCIE-16GB |
| Используемых GPU | 1 |
| Параметров codec | 3 389 222 |
...
Небольшое время опубликованного execution объясняется загрузкой уже обученного
совместимого checkpoint. Оно не является временем полного обучения модели с
нуля.
