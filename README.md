...
Дополнительно исследовалась зависимость качества реконструкции от размера обучающей выборки `N`. Сравнение результатов для `N=128` и `N=256`, а также оценка ожидаемой динамики для `N=512` и `N=1024` показали лишь небольшое снижение NRMSE и небольшой рост PSNR. При этом коэффициент сжатия оставался практически неизменным. Значения для `N=512` и `N=1024` являются экстраполяцией наблюдаемой тенденции и требуют подтверждения полноценными запусками.

Слабый прирост при увеличении `N` указывает на приближение выбранной архитектуры к насыщению: после некоторого объёма данных качество сильнее ограничивается ёмкостью автоэнкодера, функцией потерь и настройкой квантования, чем количеством обучающих кадров. Поэтому дальнейшее увеличение выборки целесообразно сочетать с улучшением архитектуры и режима обучения.
