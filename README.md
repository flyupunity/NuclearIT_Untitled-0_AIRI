...
При увеличении размера обучающей выборки `N` качество реконструкции улучшалось незначительно. Это указывает на насыщение выбранной архитектуры: дальнейшее увеличение объёма данных само по себе даёт небольшой прирост и не заменяет улучшение модели и процедуры обучения.

Отдельно был проведён эксперимент с предобученной моделью нейросетевого сжатия. Её входной и выходной слои были адаптированы с трёх изображительных каналов на 28 погодных полей, после чего модель дообучалась на данных ERA5. Такой перенос позволил использовать уже обученные пространственные фильтры и ускорить получение рабочего результата.

В исходной конфигурации предобученная модель достигла фактического сжатия примерно в `200×`. Это подтвердило работоспособность настоящего bitstream и высокий потенциал архитектуры, но такой режим оказался значительно жёстче требуемых `32×`: в поток передавалось слишком мало информации для максимально точного восстановления полей. Эксперимент показал, что предобученная модель применима к задаче, однако для основной точки `32×` требуется отдельная настройка rate–distortion баланса.
